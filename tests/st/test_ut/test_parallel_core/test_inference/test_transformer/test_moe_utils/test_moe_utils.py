...
from mindformers.parallel_core.inference.transformer.moe_utils import (
    group_limited_topk,
...
class TestTopkRoutingWithScoreFunction:
    ...
    def test_softmax_routing_returns_normalized_weights(self):
        ...
        np.testing.assert_allclose(
            expert_weight.asnumpy().sum(axis=-1),
    ...
    def test_sigmoid_routing_with_bias_without_normalization(self):
        ...
        np.testing.assert_array_equal(routing_map.asnumpy(), np.array([[3, 2]], dtype=np.int32))
        weight_sum = float(expert_weight.asnumpy().sum())
        assert weight_sum == pytest.approx(1.612, rel=1e-3)
    ...
    def test_group_limited_topk_only_selects_from_best_group(self):
        ...
        sorted_indices = np.sort(top_indices.asnumpy(), axis=-1)
        np.testing.assert_array_equal(sorted_indices, np.array([[0, 1]], dtype=np.int32))
